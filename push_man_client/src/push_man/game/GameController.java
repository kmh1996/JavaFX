...
public class GameController implements Initializable, GameInterface {
	...
	public void initField() {
		...
		Image back = new Image(
			getClass().getResource("game_image/img_back.png").toString()
		);
		Image block = new Image(
			getClass().getResource("game_image/img_block.png").toString()
		);
		Image stone = new Image(
			getClass().getResource("game_image/img_stone.png").toString()
		);
		Image houseEmpty = new Image(
			getClass().getResource("game_image/img_house_empty.png").toString()
		);
		Image houseFull = new Image(
			getClass().getResource("game_image/img_house_full.png").toString()
		);
		Image pushMan = new Image(
			getClass().getResource("game_image/img_push_man.png").toString()
		);
...
//		mGameLevelNum = 1;
	...
	public void loadDataFile(int levelNum) {
		...
		while((last = bufFile.indexOf("\n")) >= 0) {
			System.out.println("idexOf " + last);
	...
	public void showDialog(ScoreVO obj) {
		...
		op.ifPresent(t->{
			// 사용자가 버튼을 누르면 새로운 슽테이지 정보를 로딩 + 1단계
